...
For recalling these measures their formulae are provided below.
 For a recorded random continuous variable X with values x from 1 to i within its range, the interval [a,b], and having a probability distribution p(x):
\\
\\
...
\\
\\
MEDIAN \footnote{The median is the midpoint of the observed values if they are arranged in increasing order. Half of the values are below the median and half of the values are above the median. Once the data are ranked the formula for median can be applied.} 
\tab $x_{median}=$ 
\begin{cases}
$x_{(n+1)/2}$ if n odd \\
...
\\
\\
SKEWNESS \footenote{Skewness is also called the third moment about the mean. Variance is also called the second moment about the mean. And the fourth moment about the mean is the kurtosis.}
