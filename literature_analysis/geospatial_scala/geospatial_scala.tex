...
\footnote{The sum of these probabilities is 1.00. Due to rounding errors, this is not the case.},
 obtaining the information from table 2:
\begin{table} [h!]
\centering
...
Because each probability is associated with a single diameter class, table 2 practically defines a function \cite{lial_calculus_2012}.\footnote{A function is a rule that binds exactly one element from its domain to exactly one element of its codomain\cite{takahashi_mangas_2012}.}
 Such a function is called a probability function \cite{lial_calculus_2012} or a probability model \cite{pfeffer_practical_2015}.  
