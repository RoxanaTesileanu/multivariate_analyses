...
The univariate Normal distribution can be extended by adding more dimensions to an observation $\mathbf{m_i}$. 
If for a univariate Normal distribution, each observation $\mathbf{x_i}$ of a variable X can be represented as a one-dimensional vector containing one recorded variable, for bi- and multivariate Normal distributions the individual observations $\mathbf{m_i}$ can be represented as two- or multidimensional vectors containing two or more recorded variables (the set \textit{N}). 
