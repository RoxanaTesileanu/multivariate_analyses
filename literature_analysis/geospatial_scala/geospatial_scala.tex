...
In other words, when we have more than one variable recorded for each observation, say a number of k variables, we investigate them in an k-dimensional space.
 As long as they are not independent variables (all covariances are 0), they need to be analysed as associtated variables in a multivariate framework in order to predict or classify new response values \cite{jackman_bayesian_2009} \cite{carroll_mathematical_1997}.   
\\
\\    
BETA DISTRIBUTION\\
