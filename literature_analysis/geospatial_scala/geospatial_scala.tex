...
In this work I use LAPACK (Linear Algebra Package) and ATLAS (Automatically Tuned Linear Algebra Software). The later provides the Fortran interface to the portably efficient BLAS (Basic Linear Algebra Subprograms) implementation \cite{sourceforge_atlas_2017} on which LAPACK is based \cite{icl_install_2007}.  provided by the University of Tennessee, Berkeley University of California and Denver University of Colorado \cite{icl_lapack_2017} \cite{icl_blas_2017}. LAPACK and BLAS are written in Fortran and are freely-available software packages (see \href{http://www.netlib.org/lapack/}{http://www.netlib.org/lapack/}. 
\\
\\
On Ubuntu you can install LAPACK using the following commands \cite{canonical_lapack_nodate} \cite{canonical_askubuntu_nodate}:
\\
\\
