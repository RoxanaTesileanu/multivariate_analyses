...
For a bivariate Normal distribution we can write \cite{jackman_bayesian_2009}:
\begin{equation*}
\mathbf{m} = 
\begin{bmatrix}  
n_1 \\
...
\end{pmatrix}
\end{equation*}
Furthermore, the multivariate Normal density function for any k-dimensional \textbf{m} is \cite{jackman_bayesian_2009}:
\begin{equation}
p(\mathbf{m})= (2 \pi)^{-k/2} |\Sigma|^{-1/2} e^{-(\mathbf{m}- \boldsymbol{\mu})' \boldsymbol{\Sigma}^{-1} (\mathbf{m} - \boldsymbol{\mu} )/2}  
\end{equation} 
Thus, it is a function which depends on the number of dimensions (for a bivariate Normal distribution k=2), the vector of variable means ($\boldsymbol{\mu}$) and the variance-covariance matrix ($\boldsymbol{\Sigma}$).
In other words, when we have more than one variable recorded for each observation, say a number of k variables, we investigate them in an k-dimensional space.
 As long as they are not independent variables (all covariances are 0), they need to be analysed as associtated variables in a multivariate framework in order to predict or classify new response values because the observations come from a joint Normal distribution (depending on $\boldsymbol{\Sigma}$, $\boldsymbol{\mu}$ and k) \cite{jackman_bayesian_2009} \cite{carroll_mathematical_1997}.   
\\
\\    
BETA DISTRIBUTION\\
Not just the Normal distribution can be used to characterize the distribution of the residuals from the fitted models \cite{quinn_experimental_2002} \cite{pfeffer_practical_2016}. Errors can follow a binomial distribution like in the assumptions of the logistic regression, or a Poisson distribution like in the assumptions of the Poisson regression \cite{quinn_experimental_2002}.
 The Beta distribution is commonly used as a prior probability distribution for binary variables in Bayesian analyses (e.g. within the beta-binomial model \cite{jackman_bayesian_2009}, \cite{pfeffer_practical_2016}) \cite{quinn_experimental_2002} and can also be used to characterize how residuals are distributed \cite{pfeffer_practical_2016}.  
