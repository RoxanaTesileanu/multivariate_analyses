...
Briefly, it takes the linear system $LUx=b$ and sets $Ux=y$ and $Ly=b$ \cite{dawkins_pauls_2005} \cite{golub_matrix_2013}.
 This two-steps procedure makes it easier to find the solution, as finding \textbf{y} from $L\textbf{y}=\textbf{b}$ is easier because \textit{L} is a lower triangular matrix, and then knowing \textbf{y} makes it easier to solve for \textbf{x} from $U\textbf{x}=\textbf{y}$ also because it is easier to solve equations with triangular matrices than with general square matrices. 
