...
A dataset can be represented as a data matrix \textit{A} with \textbf{m} rows (each observation being represented by a row) and \textbf{n} columns (each recorded variable being represented by a column). 
Thus the data matrix \textit{A} contains \textbf{m} observations, where $m=1,..,i$, and \textbf{n} variables, where $n=1,..,j$.
 Each entry in \textit{A}, $a_{ij}$, represents the recorded value for the $i^{th}$ observation on the $j^{th}$ recorded variable.
Each observation is represented by a row vector \textbf{$m_i$} and each variable is represented by a column vector \textbf{$n_j$}.  

  and can be splitted in two or  more subsets \cite{carroll_mathematical_1997}.  
