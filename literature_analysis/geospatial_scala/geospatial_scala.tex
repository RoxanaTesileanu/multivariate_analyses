...
\\
\\
The above spatial reference is not a projected SRS, because it has a GEOCS entry only, without a PROJCS one. But, we can still see that the datum used is WGSi1984, we can the spheroid used is WGS84, the used unit for coordinate sets is degree and the authority giving the ID code is EPSG (short for European Petroleum Survey Group).  
