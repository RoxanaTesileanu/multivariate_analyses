...
\item the starting index of the flattened Array[Double] version of the \textit{A} matrix 
\item the leading dimension of \textit{A} which represents its number of rows 
\item the Array[Int] which represents a dummy array which will be overwritten by dgesv and which will store the IPIV array (the array which stores the pivot indices that define the permutation matrix \textit{P}. The IPIV(i) array shows which row was interchanged at the the ith step row \cite{nag_ltd._nag_nodate}.  
\item 
\item
\item
\item
