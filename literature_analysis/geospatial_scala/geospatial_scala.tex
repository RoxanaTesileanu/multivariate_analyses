...
\item the flattened and transposed coefficient matrix \textit{A}, \footnote{The dgesv function requires a flattened transposed coefficient matrix \textit{A} because of the following reasons: (i) the dgesv requires 1D Arrays \cite{doolin_jlapack_1999}, so the flattened \textit{A} matrix needs to be used, at the same time one must be aware that \textbf{b} represents a column vector not a row vector, thus, (ii) a transposed Scala 2D Array is used because of the fact that \textbf{b} actually should represent a column vector but is not (it is a row vector), so in order to preserve the flow of the matrix operations a transposed version of \textit{A} is used.}   
