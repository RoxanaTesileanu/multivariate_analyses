...
You could for example try out to solve the system of equations $Ax=b$, where A is a coefficient matrix (each row of A represents an observation, and each cell within an observation represents the entry for a recorded explanatory variable), x is a vector (column matrix) of unknowns from the system of equations and b is a vector (column matrix) containing the entries of the response variable (see Dawkins 2005):
