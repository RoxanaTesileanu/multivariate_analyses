...
\\
\\
In practive, even if we sample from a population and we have a distribution of values for a variable X, we are not able to construct a probability density function for X for every analysis \cite{lial_calculus_2012}. Instead, we use one of the several probability density functions which are well known and investigated in detail.   
