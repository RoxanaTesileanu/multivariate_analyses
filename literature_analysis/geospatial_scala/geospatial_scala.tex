...
\item the flattened and transposed coefficient matrix \textit{A} \footnote{the dgesv requires 1D Arrays \cite{doolin_jlapack_1999}, so the flattened \textit{A} matrix needs to be used and the Scala 2D Arrays are the transposed versions of the input Java Arrays \cite{odersky_programming_2010}}.  
\item
\item
\item
\item
