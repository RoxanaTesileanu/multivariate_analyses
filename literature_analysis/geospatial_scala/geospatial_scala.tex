...
\\
\\
You can check the routines provided by LAPLACK using the TAB-autocompletion on the lapack instance. Among these, the driver routines for solving systems of linear equations for a general matrix (xGESV) or for linear least squares problems (xGELS for QR factorization, or xGELSS for SVD factorization) \cite{anderson_lapack_1999}. You could for example try out to solve the system of equations $Ax=b$, where A is a coefficient matrix, x is a vector (column matrix) of unknowns from the system of equations and b is a vector (column matrix) containing the entries of the response variable: 
