...
At the beginning of section 3.3 the multivariate data matrix \textit{A} was introduced, with $\mathbf{m_i}$ observations (forming a set \textit{N}) and $\mathbf{n_j}$ variables (forming a set \textit{M}.
The univariate Normal distribution can be extended by adding more dimensions to an observation $\mathbf{x_i}$. 
If for a univariate Normal distribution, each observation $\mathbf{x_i}$ can be represented as a one-dimensional vector containing one recorded variable, for bi- and multivariate Normal distributions the individual observations $\mathbf{x_i}$ can be represented as two- or multidimensional vectors containing two or more recorded variables. 
For a bivariate Normal distribution, with \textbf{x\textsubscript{i}} expressed as $\mathbf{x\textsubscript{i}}= (x_1, x_2)$, the mean is expressed as a nx1 matrix $\boldsymbol{\mu}=[\mu_1, \mu_2]'$ \footnote{The nx1 matrix has the form of a column vector.}
...
For now, note that the elements of $\boldsymbol{\Sigma}$ are the covariances between the variables of X, and, that $\boldsymbol{\Sigma}$ represents the multivariate equivalent for V(x) \cite{marden_multivariate_2015}.     
