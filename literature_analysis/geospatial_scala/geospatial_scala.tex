...
The individual entries $a_{ij}$ of the data matrix \textit{A}, contain the values of the recorded variables. These values may represent continuous, discrete or categorical variables \cite{carroll_mathematical_1997}. If there is one response variable and more predictor variables, we have either multiple linear regression (if the response variable as well as the predictor variables are continuous) or multifactor MANOVA (if the response variables is continuous and the predictor variables are categorical). 
