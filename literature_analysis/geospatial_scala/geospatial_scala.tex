...
Briefly, it takes the linear system $LUx=b$ and sets $Ux=y$ and $Ly=b$ \cite{dawkins_pauls_2005} \cite{golub_matrix_2013}. These two-steps procedure makes it easier to find the solution, as finding y from $Ly=b$ is easier because it is a lower triangular matrix, and then knowing y makes it easier to solve for x from $Ux=y$ also because it is easier to solve equations with triangular matrices than with general square matrices.    
