...
Each observation is represented by a row vector \textbf{m\textsubscript{i}} (forming a set \textit{M} of observations) and each variable is represented by a column vector \textbf{n\textsubscript{j}} (forming a set \textit{N} of variables) \cite{hastie_elements_20}.  
...
For a bivariate Normal distribution, with \textbf{m\textsubscript{i}} expressed as $\mathbf{m\textsubscript{i}}= (n_{i1}, n_{i2})$, the mean is expressed as a nx1 matrix $\boldsymbol{\mu}=[\mu_1, \mu_2]'$ \footnote{The nx1 matrix has the form of a column vector.}
