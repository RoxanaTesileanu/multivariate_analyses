...
\item the "dd" command deletes the current line; "6dd" will delete the current line and the next five lines
...
A short introduction in VIM can be also found in Shotts (2009) \cite{shotts_linus_2009}, a lecture which gently introduces readers to the Linux command line in general. 
...
\subsubsubsection {Installing SBT}
...
\subsubsubsection{Creating a Scala project}

In this section we will create a Scala project called Scala\underline{\space}Playground in order to continue the playground series of Linux introductions started by William Shotts (2009) in his book "The Linux Command Line". 
\\
\\
...
\subsubsubsection {Managing dependencies}
...
\tab  groupID \%\% artifactID \% revision,\\
\tab  groupID \%\% artifactID \% revision\\
)\\
\\
\\
