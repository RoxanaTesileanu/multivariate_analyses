...
A dataset can be represented as a data matrix \textit{A} with m rows (each observation being represented by a row) and n columns (each recorded variable being represented by a column). Each entry in \textit{A}, $a_ij$, represents the recorded value for the ith observation for the jth recorded variable. 
