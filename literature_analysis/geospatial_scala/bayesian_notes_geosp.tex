...
\end{equation*}  
where $p(y)= \int p(\theta) p(y| \theta) \mathrm{d} \theta$ (for $\theta$ continuous), and $1/p(y)$ represents the constant of proportionality ensuring that the posterior density integrates to 1 \cite{jackman_bayesian_2009}. The constant of proportionality can be formally written as \cite{jackman_bayesian_2009}: 
\begin {equation*}
